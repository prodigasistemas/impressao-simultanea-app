...
public class DataManipulator {
	...
	public void selectGeral() {
		...
		Cursor cursor = db.query(Constantes.TABLE_GERAL, new String[] {
				...
				"data_referencia_arrecadada", "descricao_aliquota_imposto", "valor_aliquota_imposto",
				"descricao_aliquota_agreguladora", "valor_aliquota_agreguladora" }, null, null, null, null,
				"id asc");
		...
		if (cursor.moveToFirst()) {
			...
			getDadosGerais().setDescricaoAliquotaImposto(cursor.getString(27));
			getDadosGerais().setValorAliquotaImposto(new BigDecimal(cursor.getString(28)));
			getDadosGerais().setDescricaoAliquotaAgReguladora(cursor.getString(29));
			getDadosGerais().setValorAliquotaAgReguladora(new BigDecimal(cursor.getString(30)));
	...
	public Imovel selectDependenciasImovel(Imovel imovel){
		...
		if (imovel != null){
			...
			selecionarTarifasComplementares(imovel);
		}
		return imovel;
	}
	...
	public Imovel selectDebitos(Imovel imovel) {
		...
		Cursor cursor = db.query(Constantes.TABLE_DEBITO, new String[] {"descricao","valor","codigo","indc_uso", "indc_incluir_calculo_imposto"}, 
																		"matricula = " + imovel.getMatricula(), null, null, null, null);
		Debito debito;
		...
		if (cursor.moveToFirst()) {
			do {
				...
				debito.setIndicadorIncluirCalculoImposto(cursor.getShort(4));
	...
	public Imovel selecionarTarifasComplementares(Imovel imovel){
		logType = "selecionarTarifasComplementares";
		LogUtil.salvarLog(logType, "Imovel: " + imovel.getMatricula());
		...
		if (cursor.moveToFirst()) {
			do {
				try {
					...
					tc.setDataInicioVigencia(cursor.getString(1));
					...
					LogUtil.salvarLog(logType, "Faixa Inicial: " + tc.getLimiteInicialFaixa() 
							+ " | Faixa Final: " + tc.getLimiteFinalFaixa() 
							+ " | Valor Faixa: " + tc.getValorM3Faixa()
							+ " | Data Inicio Vigencia: " + tc.getDataInicioVigencia());
		...
		LogUtil.salvarLog(logType, "Qtd: " + imovel.getTarifacoesComplementares().size());
	...
	public List<String> selectInformacoesRota() {
		...
		ArrayList<String> list = new ArrayList<String>();
		Cursor cursor = db.query(Constantes.TABLE_IMOVEL, new String[] { "grupo_faturamento", "localidade", "setor", "codigo_rota" }, null, null, null, null,
...
//				helper.setReterImpressaoConta(cursor.getInt(12));
	...
	public long insertDadosGerais(String linhaArquivo) {
		...
		initialValues.put("descricao_aliquota_imposto", parser.obterDadoParser(30).trim());
		initialValues.put("valor_aliquota_imposto", parser.obterDadoParser(14));
		initialValues.put("descricao_aliquota_agreguladora", parser.obterDadoParser(30).trim());
		
		String valorAliquotaAgenciaReguladora = parser.obterDadoParser(14);
		initialValues.put("valor_aliquota_agreguladora", String.valueOf(valorAliquotaAgenciaReguladora == "" ? Constantes.NULO_DOUBLE : valorAliquotaAgenciaReguladora));
	...
	public boolean insertImovel(String linhaArquivo) {
		ParserUtil parser = new ParserUtil(linhaArquivo);
		parser.obterDadoParser(2);
		ContentValues initialValues = new ContentValues();
		...
		boolean informativo = ControladorImovel.getInstancia().isImovelInformativo(
	...
	public void exportDB(Context context) {
        ...
        try {
            ...
            if (sd.canWrite()) {
            	String  currentDBPath= "//data//" + "com.IS" + "//databases//" + Constantes.DATABASE_NAME;
	            
	            File currentDB = new File(data, currentDBPath);
	            File dirDB = new File(sd+"/external_sd/"+Constantes.DIRETORIO_EXPORTACAO_BANCO);
	            if(!dirDB.exists()) {
	            	dirDB.mkdirs();
	            }
	            File backupDB = new File(dirDB, Constantes.DATABASE_NAME);
	            
	            FileChannel origem = new FileInputStream(currentDB).getChannel();
	            FileChannel destino = new FileOutputStream(backupDB).getChannel();
	            ...
	            Toast.makeText(context, "Banco exportado para "+Constantes.DIRETORIO_EXPORTACAO_BANCO, Toast.LENGTH_LONG).show();
            }
        } catch (Exception e) {
	...
	public boolean isRepetido(String tabela, int tipo) {
		Cursor cursor = db.query(tabela, null, null, null, getGroupByValidacao(tipo), "count(*) > 1", null);
	...
	private String getGroupByValidacao(int tipo) {
		String groupBy = "matricula";
		
		switch (tipo) {
		case Constantes.REGISTRO_TIPO_DADOS_CATEGORIA:
			groupBy += ", codigo_categoria, descricao_categoria";			
			break;
			
		case Constantes.REGISTRO_TIPO_HISTORICO_CONSUMO:
			groupBy += ", ano_mes_referencia";
			break;
		
		case Constantes.REGISTRO_TIPO_IMPOSTO:
			groupBy += ", tipo_imposto, descricao_imposto, percentual_aliquota";
			break;
			
		case Constantes.REGISTRO_TIPO_CONTA:
			groupBy += ", ano_mes_referencia_conta, valor_conta";
			break;
			
		case Constantes.REGISTRO_TIPO_TARIFACAO_MINIMA:
			groupBy += ", data_vigencia, codigo_categoria, tarifa_minima_categoria";
			break;
			
		case Constantes.REGISTRO_TIPO_TARIFACAO_COMPLEMENTAR:
			groupBy += ", data_inicio_vigencia, codigo_categoria, valor_m3_faixa";
			break;
	...
	public long insertDadosRota(String[] dados) {
		ContentValues initialValues = new ContentValues();
		initialValues.put("grupo", dados[0]);
		initialValues.put("localidade", dados[1]);
		initialValues.put("setor", dados[2]);
		initialValues.put("rota", dados[3]);
		initialValues.put("referencia", dados[4]);

		return db.insert(Constantes.TABLE_DADOS_ROTA, null, initialValues);
	}
	
	public List<String> selectDadosRota() {
		Cursor cursor = db.query(Constantes.TABLE_DADOS_ROTA, new String[] { "grupo", "localidade", "setor", "rota", "referencia" }, null, null, null, null, null);
